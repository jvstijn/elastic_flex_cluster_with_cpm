# Docker Elastic Stack (CPM local demo)
...
```bash
cd ../elastic_flex_cluster_with_cpm/ansible
ansible-playbook -i inventories/local site.yml
ansible-playbook -i inventories/local bootstrap.yml
```

Requires Stack Monitoring data in `.monitoring-es-8-*` on es-central. Cluster registry for the local inventory is in `ansible/inventories/local/group_vars/all.yml`.

Optional backfill for ML history:

```bash
python3 scripts/backfill_monitoring_history.py
```
...
python3 ../docker-local/scripts/bootstrap_cpm_pipelines.py
```
...
```bash
python3 scripts/simulate_auditd_load.py      # 1M docs → central
python3 scripts/simulate_nginx_load.py       # nginx → remote-a
