...
| ID | Testtype | Prio | Testscenario | Preconditie | Teststappen | Verwacht resultaat | Werkelijk resultaat | Status |
|----|----------|------|--------------|-------------|-------------|--------------------|---------------------|--------|
| FUNC-RS-01 | Functioneel | Hoog | register-sync vult registry met alle 7 clusters | INFRA-02/03 | 1. `run_wf cpmw-register-sync` <br>2. `$EC "$ES/cpmw-cluster-registry/_search?size=20"` | Alle steps `completed`; **7** docs; per cluster correcte `disk_total_bytes` (~85% som node-disk), `heap_max_bytes`, `node_count`, `dc` (`unknown` of uit inventory), niet-lege `ingest_hosts`, `active:true`; **geen** `shard_max_threshold` / `write_queue_threshold` in registry | | Open |
...
| FUNC-PM-01 | Functioneel | Hoog | pipeline-manager pusht gerenderde Logstash-pipelines | FUNC-SM-01/02 | 1. `run_wf cpmw-pipeline-manager` <br>2. `$EC "$ES/_logstash/pipeline"` <br>3. Inspecteer 1 pipeline-config | Pipelines aangemaakt per `pipeline_id`; **geen** `__TOKEN__` over; `topics`, `hosts => ["<host>:9200"]`, `group_id => "cpm-ingest"` (of waarde uit `_global`), `api_key => "${ES_API_KEY_<ID>}"` correct; `pipeline_settings` aanwezig | | Open |
...
| ID | Testtype | Prio | Testscenario | Preconditie | Teststappen | Verwacht resultaat | Werkelijk resultaat | Status |
|----|----------|------|--------------|-------------|-------------|--------------------|---------------------|--------|
| ENV-01 | Coexistence | Midden | Watchers en workflows storen elkaar niet | Beide actief | 1. `$EC "$ES/_watcher/stats"` <br>2. Draai cpmw-keten <br>3. Vergelijk `cpm-*` vs `cpmw-*` indices | 7 watchers actief; workflows schrijven uitsluitend naar `cpmw-*`; `cpm-*` indices onveranderd door workflows (en omgekeerd) | | Open |
